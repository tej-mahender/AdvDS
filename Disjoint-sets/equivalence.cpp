...
int main() {
    ...
    cout << "Enter pairs to check equivalence (enter -1 to stop):" << endl;
    while (true) {
    int x, y;
    cout << "Enter pair: ";
    cin >> x;
    if (x == -1)
        break;
    cin >> y;
    if (simpleFind(x) == simpleFind(y))
        cout << x << " and " << y << " belong to the same equivalence set." << endl;
    else
        cout << x << " and " << y << " do not belong to the same equivalence set." << endl;
    }
    return 0;
}
